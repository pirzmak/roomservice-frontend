...
class CalendarGrid extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.reservations !== this.props.reservations) {
      this.setState({reservations: nextProps.reservations}, () => this.filterReservations());
    }
  }

  filterReservations() {
    const newReservationModel = this.state.reservations.filter(d => {
      ...
      <div className="calendarGrid">
        <div className="calendarGridOptions">
          ...
          <div className="calendarGridOptionsRight">
            <div
              className={"calendarGridOptionsRow calendarGridOptionsRowButton" + (this.state.diff === 3 ? " active" : "")}
              onClick={
                () => this.setState({
                  diff: 3,
                  from: getMonthDay(now(), 1),
                  to: now().add(2, 'M').endOf('month')
                })}>3 miesiące
            </div>
            <div
              className={"calendarGridOptionsRow calendarGridOptionsRowButton" + (this.state.diff === 1 ? " active" : "")}
              onClick={
                () => this.setState({
                  diff: 1,
                  from: getMonthDay(now(), 1),
        ...
        {this.props.rooms.length > 0 ? this.state.reservationsViewModel.map((r, i) => <ReservationRect
          key={Math.random()}
          reservationId={r.id}
          fromDay={r.fromDay}
          toDay={r.toDay}
